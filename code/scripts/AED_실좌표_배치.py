...
세 가지 요구를 반영한다.
  ...
  3) 기존 AED — 이미 야간 커버되는 곳에 중복 배치하지 않는다.
...
def load_existing_aed(aed_osm):
    """공공데이터포털 산출물 > OSM 순으로 기존 AED 좌표를 확보한다."""
    ...
    if aed_osm:
        ...
        return aed_osm, "OSM"
    print("[기존AED] ⚠️ 좌표 없음 — 노트북 11 의 '야간 미커버 고위험 집계구'로 대체")
    return [], "미확보"
...
def main():
    ...
    if aed_xy:
        ...
        # 폴백: 기존 AED 좌표가 없으면 노트북 11 이 고른 15개(야간 미커버·고위험)를
        # '확실히 안 닿는 곳'으로 보고 가중을 올린다. 나머지는 제외하지 않고
        # 그대로 두되 우선순위만 낮춘다 — 전수 제외는 수요를 과소평가한다.
